...
dag = DAG('hotel_overall_gx_data', description='hotel_overall_gx_data from hive to ranking service ',
...
################ overall_gx score ##############
def get_hive_data_overall_gx(**kwargs):
    with open(file) as f:
        query = f.read()
    cursor = conn.cursor()
...
def kafka_event_overall_gx(**kwargs):
    ti = kwargs['ti']
    data_city = ti.xcom_pull(task_ids='get_hive_data_overall_gx')
    for d in data_city:
        x = create_json_overall_gx(d)
        producer.send('acp_corp_airflow_hotel_data', json.dumps(x))
        print(x)


hive_task_overall_gx = PythonOperator(
    task_id='get_hive_data_overall_gx',
    python_callable=get_hive_data_overall_gx,
    provide_context=True,
    dag=dag
)

kafka_task_overall_gx = PythonOperator(
    task_id='kafka_event_overall_gx',
    python_callable=kafka_event_overall_gx,
    provide_context=True,
    dag=dag
)
###########################################
...
hive_task_overall_gx >> kafka_task_overall_gx >> op